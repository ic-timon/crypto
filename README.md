...
**Android 客户端（Jetpack Compose + Material 3）** 与 **原生密码学库 `mobi.timon.crypto`** 的同仓工程：应用侧用 Kotlin 写 UI 与业务壳，加解密、哈希、KDF、非对称与签名等能力集中在 **`:crypto`** 模块，经 **Kotlin → JNI（C）→ Go（`crypto/*`、`golang.org/x/crypto`）** 落地，便于与 Go 生态实现、测试向量及多端行为对齐。
...
| **`:crypto`** | 可复用的 Android Library（`mobi.timon.crypto`）：对外暴露 Kotlin API，内部通过 JNI 调用 Go 编译出的 `libencgo.so`，与 `libencjni.so` 协同加载。 |
...
| `:crypto` | `mobi.timon.crypto` | 密码学工具库（见下节）。 |
...
- **加载顺序**（见 `crypto/.../Enc.kt`）：先 **`encgo`**，再 **`encjni`**。
- **构建**：Gradle 任务（如 `compileGoDebug`）为各 ABI 生成 **`libencgo.so`** 与 **`libencgo.h`**；CMake 按 ABI 链接；输出一般在 `crypto/src/main/go/lib/` 等约定目录（勿提交生成的 `.so` 到版本库，以本地/CI 构建为准）。

### API 一览

失败时统一抛出 **`mobi.timon.crypto.EncException`**（含 JNI / Go 错误路径）。
...
| **scrypt** | Go 侧 **N、r、p 固定**；Kotlin 侧重在 `keyLen`（见 `crypto/.../kdf_advanced.go`）。 |
...
# 仅编译 crypto 的 Go 产物（各 ABI）
./gradlew :crypto:compileGoDebug

# 构建依赖 crypto 的应用（会触发 preBuild → compileGo*）
...
`:crypto` 中与 JNI、Codec（如 Android `Base64`）相关的用例放在 **`crypto/src/androidTest`**；需能加载对应 ABI 的 `.so`。
...
1. `crypto/src/main/java/mobi/timon/crypto/*.kt` 中的 **`external fun`**
2. `crypto/src/main/cpp/enc_jni.c` 中的 **`Java_mobi_timon_crypto_*`**
3. `crypto/src/main/go/*.go` 中的 **`//export`** 与构建产物 **`libencgo.h`**
...
.
├── app/                          # 主应用（Compose）
├── crypto/                       # 密码学 Android Library
│   ├── build.gradle.kts
│   ├── enc.md                    # 算法选型与设计备忘（可选读）
│   ├── src/main/java/mobi/timon/crypto/
