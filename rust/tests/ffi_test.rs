...
fn random_bytes_works() {
    ...
    let r = encrust::random::RandomBytes(32, &mut out_len);
...
fn aes_gcm_roundtrip() {
    ...
    let mut out_len: i32 = 0;
    let ct = encrust::aead::AesGcmEncrypt(pt.as_ptr(), pt.len() as i32, key.as_ptr(), 32, &mut out_len);
...
fn aes_cbc_roundtrip() {
    ...
    let mut out_len: i32 = 0;
    let ct = encrust::cbc::AesCbcEncrypt(pt.as_ptr(), pt.len() as i32, key.as_ptr(), 16, &mut out_len);
    ...
    let mut out2: i32 = 0;
    let dec = encrust::cbc::AesCbcDecrypt(ct_vec.as_ptr(), ct_len, key.as_ptr(), 16, &mut out2);
